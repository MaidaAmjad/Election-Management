...
begin
  ...
  if p_start_datetime is null or p_end_datetime is null then
    return jsonb_build_object('success', false, 'message', 'Start and end date/time are required.');
  end if;
  ...
  if v_row.registration_deadline >= p_start_datetime then
    return jsonb_build_object(
      'success',
      false,
      'message',
  ...
  if v_row.registration_status = 'Finalized' and p_max_voters is distinct from v_row.max_voters then
    return jsonb_build_object(
      'success',
      false,
      'message',
...
grant execute on function public.update_approved_election_schedule(uuid, timestamptz, timestamptz, integer)
  to authenticated;
